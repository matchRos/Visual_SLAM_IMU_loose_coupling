...
% and IMU fusion by using root-mean-square error(RMSE), in order to 
% evaluate the accuracy of visual SLAM. The value of variable 'ATE' is the 
% ATE of visual SLAM.
% Author: Chen, Li
% Email: hustchenli617@gmail.com
% Date: 13-Jan-2021
%% Select dataset
% This part is to select data from experiment, 'ekf09114.bag' is
% experiment data with faster movement, 'ekf09118.bag' is experiment data
% with slower movement.
clear;
slam_name = 'ekf09114.bag';
% slam_name = 'ekf09118.bag';
%% Set beginning and end
% Since fusion and FARO measurement cannot be started and ended exactly at
% the same time. There are preparation periods by the beginning and the
% end of fusion and FARO measurement, which are supposed to be removed before
% the calculation of ATE.
if slam_name == 'ekf09114.bag'
    meas_name = 'test09114.xlsx';
elseif slam_name == 'ekf09118.bag'
    meas_name = 'test09118.xlsx';
else
    error('Please enter the correct file name.');
end
bag = rosbag(slam_name);
bSel = select(bag,'Topic','/robot_pose_ekf/odom_combined');
...
ATE = sqrt(sum(Er.^2)/x_slam_T_len);
%% Plot the results
% This part is to plot results of visual SLAM IMU fusion as well as FARO 
...
ylabel('x [m]')
title('ORB-SLAM fused with IMU x axis');
figure;
...
xlabel('number of points');
ylabel('y [m]')
title('ORB-SLAM fused with IMU y axis');
figure;
...
xlabel('number of points');
ylabel('z [m]')
title('ORB-SLAM fused with IMU z axis');